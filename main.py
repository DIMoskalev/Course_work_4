from src.utlis import work_with_vacancies_from_api, work_with_vacancies_from_json
...
def main():
    ...
    while True:
        start_input = input('Чтобы начать работу программы нажмите Enter и следуйте инструкциям.\n'
                            ...
                            'Для того чтоб вернутся к выбору операций введите "back" или "назад".\n')
        while start_input not in ['stop', 'стоп']:
            user_input = input('Выберите, в каком виде вы хотите работать с вакансиями.\n'
                               '1 - Программа предоставит возможность работать с вакансиями с сайта hh.ru\n'
                               '2 - Программа предоставит возможность работать с вакансиями из json-файла\n').lower()
            if user_input == '1':
